 ...
 * @params {Object} wrecked - info from wrecked vote
 * @return {String}
 */
function createMessage( stats, users, meta, lottery, wrecked ) {
  const slackMsg = [];
  ...
  slackMsg.push({
    ...
    text: {
      type: 'mrkdwn',
      text: `:goldenpoop:  *Pissfingers:  ${stats.lowest.slack.real_name}*`
    }
  });
  ...
  if ( wrecked.chosen ) {
    const { chosen: { slack }, votes, amount } = wrecked;
    slackMsg.push(
      {
        type: 'section',
        text: {
          type: 'mrkdwn',
          text: `:getrekt: ${slack.real_name}, you have been chosen by ${votes} of your peers. ${amount} has been taken from you.`,
        }
      }
    );
  }
