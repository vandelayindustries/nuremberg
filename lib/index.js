...
function createMeta( messages, users ) {
  ...
  for ( const { text, user, sentiment } of messages ) {
    ...
    if (!metaDataByUser[ user ]) {
      console.log('weird user alert', user);
    }
...
function newGuacEventOp( to, from, amount, type ) {
  // Don't create events for 0 amounts; they're useless and we don't need to
  // track them.
  if ( amount === 0 ) {
    return;
  }
...
/**
 * wreck queries the DB for the wreck votes and determines if there was a
 * majority vote for someone. If there was, that user's guac balance is halved.
 * Upon a tie, e.g. two users receive 1 vote, no guac is taken from the users.
 *
 * If a user was chosen, that user, the number of votes, and the amount taken
 * is returned.
 *
 * @returns {Promise<Object>} { chosen: User|null, votes: Number, amount: Number }
 * @throws
 */
async function wreck() {
  const votes = await db.db.collection('wreck_votes').find().toArray();

  // nobody voted
  if ( !votes.length ) {
    return { chosen: null, votes: 0, amount: 0 };
  }

  let chosenSlackName = null;
  let highestVoteCount = 0;

  votes.reduce( ( tallies, vote ) => {
    let count = ( tallies.get( vote.target ) || 0 ) + 1;

    // To track if there's currently a tie. When a newly counted vote makes a
    // user's total vote count equal to the highestVoteCount AND the chosen user
    // is not this vote's target, that mean's two users have the same vote
    // count for now. By setting chosen to null and keeping the
    // highestVoteCount, we can mark that there was a tie and still track the
    // highest vote count.
    if ( count === highestVoteCount && chosenSlackName !== vote.target ) {
      chosenSlackName = null;
    }

    if ( count > highestVoteCount ) {
      highestVoteCount = count;
      chosenSlackName = vote.target;
    }

    tallies.set( vote.target, count );
    return tallies;
  }, new Map() );

  // Nobody had the highest number of votes.
  if ( chosenSlackName === null ) {
    await db.db.collection('wreck_votes').remove({});
    return { chosen: null, votes: 0, amount: 0 };
  }

  // Get the updated user doc in order to get the user's up-to-date guac_total.
  const chosen = await db.db.collection('users').findOne({
    'slack.name': chosenSlackName
  });

  const half = Math.ceil( chosen.guac_total / 2 );
  const wageAdj = wageAdjustmentOp( chosen.slack.id, half );
  const ge = newGuacEventOp( 'sentiment', chosen.slack.id, half, 'wrecked' );

  await Promise.all([
    db.db.collection('users').bulkWrite([ wageAdj ]),
    db.db.collection('guac_events').bulkWrite([ ge ]),
    db.db.collection('wreck_votes').remove({}),
  ]);

  return { chosen, votes: highestVoteCount, amount: half };
}
...
async function updateGuac( users, meta, winners, losers ) {
  ...
  // newGuacEventOp returns undefined if the amount is === 0. Filtering those out
  // so it doesn't create useless events for 0 amounts.
  evtOps = evtOps.filter( x => x );
...
/**
 * Picks a random user, gets the amount of guac in the pot, gives the user that
 * amount, then resets the pot amount. A guac event is created when giving the
 * user the lottery prize. If there's nothing in the pot or there's new users
 * given, it returns { winner: null, amount: 0 }
 *
 * @param {Object} users - slack id mapped to user
 * @returns {Promise<Object>} { winner: User|null, amount: Number }
 * @throws
 */
async function playLottery( users ) {
  if ( !Object.keys( users ).length ) {
    return { winner: null, amount: 0 };
  }

  const g = await db.db.collection('global').findOne({});
  ...
  const u = vals[ Math.floor( Math.random() * Math.floor( vals.length ) ) ];

  // Get the updated user doc in order to get the user's up-to-date guac_total.
  const winner = await db.db.collection('users').findOne({ _id: u._id });

  const wageAdj = wageAdjustmentOp( u.slack.id, g.pot + winner.guac_total );
  const ge = newGuacEventOp( u.slack.id, 'sentiment', g.pot, 'won-lottery' );

  await Promise.all([
    db.db.collection('users').bulkWrite([ wageAdj ]),
    db.db.collection('guac_events').bulkWrite([ ge ]),
    db.db.collection('global').updateOne({ _id: g._id }, { $set: { pot: 0 } })
  ]);

  return { winner, amount: g.pot };
}
...
async function handler() {
  try {
    ...
    console.log('fetching slack profiles');
    const profiles = await fetchSlackProfiles( messages );
    console.log('saving slack profiles');
    ...
    console.log('updating guac');
    await updateGuac( users, meta, stats.winners, stats.losers );
    console.log('playing lottery');
    const lotteryInfo = await playLottery( users );
    console.log('check if someone gets wrecked');
    const wreckedInfo = await wreck();
    ...
    const msg = createMsg( stats, profiles, meta, lotteryInfo, wreckedInfo );
