...
async function wreck( users ) {
  ...
  votes.reduce( ( tallies, vote ) => {
    ...
    // highest vote count.
    if ( count === highestVoteCount && chosenSlackID !== vote.target ) {
      chosenSlackID = null;
    }

    if ( count > highestVoteCount ) {
      highestVoteCount = count;
      chosenSlackID = vote.target;
    }
  ...
  const chosen = await db.db.collection('users').findOne({
    'slack.name': chosenSlackID
  });

  const amt = Math.ceil( chosen.guac_total / 2 );
  const wageAdj = wageAdjustmentOp( chosenSlackID, chosen.guac_total - amt );
  const ge = newGuacEventOp( 'sentiment', chosenSlackID, amt, 'wrecked' );
  ...
  return { chosen, votes: highestVoteCount, amount: amt };
}
