...
function wageAdjustmentOp( slackID, amount ) {
  return {
    updateOne: {
      filter: {
        'slack.id': slackID//user.slack.id
      },
      update: {
        $set: {
          guac_total: amount //user.guac_total + amount
        }
      }
    }
  };
...
async function updateGuac( users, meta, winners, losers ) {
  ...
  for ( const [ id, m ] of Object.entries( meta ) ) {
    ...
    evtOps.push( newGuacEventOp( to, from, Math.abs( amount ) ) );
  }
  
  // for now filter any non-numeric wages
  // TODO: update Operator to enforce only integer wagers
  ...
  for ( const [ user, wager ] of winners ) {
    if ( Number.isInteger( wager ) ) {
      ...
      ops.push( wageAdjustmentOp( user.slack.id, adjusted ) );
      evtOps.push( newGuacEventOp( user.slack.id, 'sentiment', winnings ) );
    }
  }
